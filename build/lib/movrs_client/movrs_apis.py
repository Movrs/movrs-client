...
def login_user(email, password):
    ...
    data = {"email": email, "password": password, "app_name": "movrs-client"}
    ...
    try:
        ...
        update_or_create_json(os.path.join(BASE_DIR, "movrs-read.json"), api_key_data)
        update_json_fields(
            [
...
def update_or_create_json(file_path: str, new_data):
    """
    ...
    if os.path.exists(file_path):
        ...
        data = {}
    # If new_data is a JSON string, parse it
    if isinstance(new_data, str):
        try:
            new_data = json.loads(new_data)
        except json.JSONDecodeError:
            print("Error: api_key is not a valid JSON string.")
            return {}

    # Correctly format the private key by replacing escaped newlines
    if new_data and "private_key" in new_data and isinstance(
        new_data.get("private_key"), str
    ):
        new_data["private_key"] = new_data["private_key"].replace("\\n", "\n")
...
def create_json_file(filepath="current_state.json"):
...
def run_docker_compose(detach=True, filepath=None):
    ...
    if missing_images:
        print("Error: Missing Docker images:", missing_images)
        print("The required Docker images are not present locally. The updater should download them.")
        return
    ...
    try:
        ...
        with (
            open(stdout_log_path, "wb") as stdout_log,
            open(stderr_log_path, "wb") as stderr_log,
        ):
