...
class UpdateWorker(QObject):
    ...
    def run(self):
        if self.version:
            result = update_to_version(self.version)
        else:
            result = confirm_version_check()
        self.finished.emit(result)


class ControlPanel(QWidget):
    def __init__(self, login_window, version=None):
        super().__init__()
    ...
    def start_update(self):
        ...
        self.worker = UpdateWorker(self.version)
    ...
    def on_update_finished(self, result):
        ...
        if "up to date" in result or "updated" in result:
            self.toggleProcess()
        else:
            self.status_label.setText(f"Update failed: {result}")
